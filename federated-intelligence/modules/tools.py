...
async def tool_consult_model(args: dict) -> list[TextContent]:
    ...
    try:
        ...
        if session_id:
            ...
            if not session:
                # Create new session if ID provided but not found? Or fail?
                # Let's create a new one if it doesn't exist, or maybe just fail.
                # Better: if user provides session_id, they expect it to exist or be created.
                # But usually session_id is generated by the server.
                # Let's assume if they pass an ID, they want to resume.
                # If they want a new session, they should call create_session (which we haven't exposed yet)
                # OR we can say: if session_id is "new", create one.
                if session_id == "new":
